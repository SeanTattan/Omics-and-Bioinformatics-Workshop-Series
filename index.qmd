...
Now lets add another and overlay a boxplot onto the plot
...
Can you use your `ggplot2` skills to create a custom visualization of the dataset?
...
In the last workshop activity, we ran a differential gene expression analysis using `DEseq2` and extracted a `.txt` file which contained a list of genes that are signficantly upregulated (log2 FC >=1 and padj < 0.05) in the NTCU-Induced squamous cell carcinoma samples.

In this workshop, we will use `StringDB` in the browser to perform a gene ontology (GO) analysis. GO analysis helps us identify which biological processes and molecular functions are enriched among the upregulated genes.

## Navigate to Stringdb 

Firstly navigate to the `Stringdb` website https://string-db.org/. Then select the `Browse` button as higlighted in the text below
